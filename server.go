...
type conn struct {
	...
	// tlsFingerprint is the TLS ClientHello fingerprint of the connection,
	// computed after the handshake. nil for plaintext connections.
...
func (c *conn) readRequest(ctx context.Context) (w *response, err error) {
	...
	if c.tlsFingerprint != nil {
		req.Fingerprint = &Fingerprint{TLS: c.tlsFingerprint}
	}
...
func (c *conn) serve(ctx context.Context) {
	...
	if tlsConn, ok := c.rwc.(*tls.Conn); ok {
		...
		*c.tlsState = tlsConn.ConnectionState()
		// Capture the TLS ClientHello fingerprint. The uTLS fork records
		// the raw ClientHello on *tls.Conn (and *tls.UConn, which embeds
		// it) during the handshake; the capture is skipped for any other
		// conn type.
		if tlsFingerprintConn, ok := tlsConnOf(c.rwc); ok {
			c.tlsFingerprint = FingerprintFromTLSConn(tlsFingerprintConn)
		}
