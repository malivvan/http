...
import (
	...
	"github.com/malivvan/http/httpproxy"
)
...
type Transport struct {
	...
	// ClientHelloID, if set, makes the Transport impersonate the TLS
	// ClientHello of a real client (a browser, usually) during the TLS
	// handshake, using github.com/malivvan/tls (a uTLS fork). This is the
	// primary mechanism to avoid TLS fingerprinting (JA3/JA4) by remote
	// servers.
	//
	// Set it to one of the pre-defined IDs such as tls.HelloChrome_Auto,
	// tls.HelloFirefox_Auto, tls.HelloSafari_Auto, or build a custom
	// ClientHelloSpec with tls.UClient and Transport.DialTLSContext for
	// full control.
	//
	// If unset (the zero value), the Transport performs a standard TLS
	// handshake, exactly like net/http.
	//
	// Prefer creating clients with NewClient, NewChromeClient,
	// NewFirefoxClient, NewSafariClient, NewIOSClient, NewEdgeClient or
	// NewOperaClient, which configure a Transport with a ClientHelloID and
	// matching HTTP/2 behavior out of the box.
	ClientHelloID tls.ClientHelloID

	// ForceHTTP1 disables HTTP/2 negotiation when ClientHelloID is set.
	// It removes "h2" from the ALPN extension of the impersonated
	// ClientHello and prevents the automatic HTTP/2 upgrade of the
	// Transport, forcing HTTP/1.1 connections.
	ForceHTTP1 bool

	// WithRandomTLSExtensionOrder, when ClientHelloID is set, shuffles
	// the order of the TLS extensions of the impersonated ClientHello
	// randomly for every connection. This makes connections of the same
	// process harder to link, at the cost of deviating from the exact
	// browser fingerprint.
	WithRandomTLSExtensionOrder bool

	// HTTP2Settings, if non-nil, customizes the initial SETTINGS frame
	// sent to HTTP/2 servers. The settings are sent in the order given.
	// When nil, the default net/http settings are sent (plus
	// HTTP2HeaderTableSize, HTTP2InitialWindowSize and
	// HTTP2MaxHeaderListSize if set).
	//
	// Browsers send their settings in a fixed order with fixed values;
	// setting this to the browser's exact SETTINGS frame (see
	// NewChromeClient and friends) is a core part of mimicking a browser
	// over HTTP/2.
	//
	// HTTP2HeaderTableSize and HTTP2InitialWindowSize take precedence over
	// entries of the same ID in HTTP2Settings.
	HTTP2Settings []Setting

	// HTTP2HeaderTableSize sets SETTINGS_HEADER_TABLE_SIZE (RFC 7540
	// Section 6.5.2) in the initial SETTINGS frame of HTTP/2 connections.
	// Zero means the net/http default (4096) is used.
	HTTP2HeaderTableSize uint32

	// HTTP2InitialWindowSize sets SETTINGS_INITIAL_WINDOW_SIZE (RFC 7540
	// Section 6.5.2) in the initial SETTINGS frame of HTTP/2 connections.
	// Zero means the net/http default (65535) is used.
	HTTP2InitialWindowSize uint32

	// HTTP2MaxHeaderListSize sets SETTINGS_MAX_HEADER_LIST_SIZE (RFC 7540
	// Section 6.5.2) in the initial SETTINGS frame of HTTP/2 connections.
	// Zero means the net/http default (10MB) is used.
	HTTP2MaxHeaderListSize uint32

	// HTTP2Priorities, if non-nil, sends the given PRIORITY frames
	// immediately after the initial SETTINGS frame of each HTTP/2
	// connection, in order. Browsers use a fixed set of PRIORITY frames at
	// connection start, which is part of their HTTP/2 fingerprint.
	HTTP2Priorities []Priority

	// HTTP2HeaderPriority, if non-nil, attaches the given stream priority
	// to the HEADERS frame of every HTTP/2 request. Browsers set a fixed
	// priority on their request headers.
	HTTP2HeaderPriority *PriorityParam

	// HTTP2ConnectionFlow is the connection-level flow control window to
	// advertise with the initial WINDOW_UPDATE frame of each HTTP/2
	// connection. Zero means the net/http default (1<<30). Browsers
	// advertise a fixed connection window (e.g. 15663105 for Chrome),
	// which is part of their HTTP/2 fingerprint.
...
func (t *Transport) onceSetNextProtoDefaults() {
	...
	// ForceHTTP1 disables HTTP/2 negotiation: the documented way to do so
	// is a non-nil, empty TLSNextProto.
	if t.ForceHTTP1 {
...
func (pconn *persistConn) addTLS(name string, trace *httptrace.ClientTrace) error {
	...
	// If the Transport is configured with a ClientHelloID, impersonate the
	// TLS fingerprint of a real client (browser) via the uTLS fork. The
	// resulting *tls.UConn embeds a *tls.Conn, so it remains usable
	// everywhere a *tls.Conn is expected (see tlsConnOf).
	//
	// Note that UConn overrides Handshake: it must be called on the UConn
	// itself (not on the embedded *tls.Conn) or the impersonated
	// ClientHello would not be sent.
	var tlsConn *tls.Conn // underlying *tls.Conn, for ConnectionState and the h2 upgrade path
	var handshaker interface{ Handshake() error }
	if clientHelloIDSet(pconn.t.ClientHelloID) {
		uconn := tls.UClient(plainConn, cfg, pconn.t.ClientHelloID, pconn.t.WithRandomTLSExtensionOrder, pconn.t.ForceHTTP1, false)
		pconn.conn = uconn
		tlsConn = uconn.Conn
		handshaker = uconn
	} else {
...
// clientHelloIDSet reports whether id is a non-zero ClientHelloID.
//
// Note: github.com/malivvan/tls's own ClientHelloID.IsSet method is
// inverted (it returns true only for the zero value), so it cannot be used.
func clientHelloIDSet(id tls.ClientHelloID) bool {
	return id.Client != "" || id.Version != "" || id.SpecFactory != nil || id.Seed != nil || id.Weights != nil
}

// tlsConnOf returns the *tls.Conn embedded in c, if c is a TLS connection
// (possibly wrapped in a *tls.UConn by the uTLS fork). It reports false
// otherwise.
func tlsConnOf(c net.Conn) (*tls.Conn, bool) {
	switch c := c.(type) {
	case *tls.Conn:
		return c, true
	case *tls.UConn:
...
func (t *Transport) dialConn(ctx context.Context, cm connectMethod) (pconn *persistConn, err error) {
	...
	if cm.scheme() == "https" && t.hasCustomTLSDialer() {
		...
		if tc, ok := tlsConnOf(pconn.conn); ok {
	...
	if s := pconn.tlsState; s != nil && s.NegotiatedProtocolIsMutual && s.NegotiatedProtocol != "" {
		if next, ok := t.TLSNextProto[s.NegotiatedProtocol]; ok {
			tconn, ok := tlsConnOf(pconn.conn)
			if !ok {
				// The custom dialer returned a conn that is not a
				// *tls.Conn (and not a *tls.UConn); fall back to
				// HTTP/1.x on it.
				return pconn, nil
			}
			alt := next(cm.targetAddr, tconn)
