...
type Transport struct {
	...
	// ClientHello of a real client (browser) during the TLS handshake,
	// using github.com/malivvan/tls (a uTLS fork), instead of a standard
	// tls.Dial. This is the primary mechanism to avoid TLS fingerprinting
	// (JA3/JA4) by remote servers.
	//
	// Set it to one of the pre-defined IDs such as tls.HelloChrome_Auto,
	// tls.HelloFirefox_Auto or tls.HelloSafari_Auto. It is ignored when
	// DialTLS is set.
...
// tlsConn is the subset of *tls.Conn (and *tls.UConn, which embeds it) used
// by dialTLSDefault.
...
func (t *Transport) dialTLSDefault(network, addr string, cfg *tls.Config) (net.Conn, error) {
	var cn tlsConn
	if clientHelloIDSet(t.ClientHelloID) {
		// Impersonate the TLS ClientHello of a real client (browser)
		// during the handshake, to avoid TLS fingerprinting (JA3/JA4).
		rawConn, err := net.Dial(network, addr)
		if err != nil {
			return nil, err
		}
		cn = tls.UClient(rawConn, cfg, t.ClientHelloID, false, false, false)
	} else {
		var err error
...
func (t *Transport) newClientConn(c net.Conn, addr string, singleUse bool) (*ClientConn, error) {
	...
	// MTU + github.com/malivvan/tls record padding.
...
func (cc *ClientConn) encodeHeaders(req *http.Request, addGzipHeader bool, trailers string, contentLength int64) ([]byte, error) {
	...
	enumerateHeaders := func(f func(name, value string)) {
		...
		if len(pHeaderOrder) == 0 {
			// Neither the request nor the Transport specified a
			// pseudo-header order: fall back to the default Go order
