...
func Test_slidingWindow(t *testing.T) {
	...
	for range testCount {
		t.Run("", func(t *testing.T) {
			...
			assert.Equal(t, windowLength, cap(sw.buf), "window length")
...
// This is a regression test for https://github.com/malivvan/http/websocket/issues/435
func TestWriteSingleFrameCompressed(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, opText, h.opcode, "opcode")
			assert.Equal(t, tc.wantRsv1, h.rsv1, "rsv1 (compressed)")
			assert.Equal(t, true, h.fin, "fin")
...
func TestWriteThenWriterContextTakeover(t *testing.T) {
	...
	assert.Equal(t, MessageText, r.typ, "Write type")
	assert.Equal(t, string(msg1), string(r.p), "Write content")
	...
	r = <-readCh
	assert.NoError(t, r.err)
	assert.Equal(t, MessageBinary, r.typ, "Writer type")
	assert.Equal(t, string(msg2), string(r.p), "Writer content")
