...
type AcceptOptions struct {
	...
	// OnRequest is an optional callback invoked synchronously after the
	// handshake passes validation and origin verification but before the
	// upgrade response is written. It receives the request for
	// fingerprinting; all request details including headers, RemoteAddr
	// and TLS state are available.
	//
	// If the callback returns an error, Accept responds with 403
	// Forbidden and the error message, and the connection is not
	// upgraded. The callback must not modify the request.
