...
import (
	...
	"github.com/malivvan/http/websocket/wsjson"
...
func TestConn(t *testing.T) {
	...
	t.Run("badClose", func(t *testing.T) {
		...
		err := c1.Close(-1, "")
		assert.ErrorContains(t, err, "failed to marshal close frame: status code StatusCode(-1) cannot be set")
	})
	...
	t.Run("badPing", func(t *testing.T) {
		...
		err := c1.Ping(ctx)
		assert.ErrorContains(t, err, "failed to wait for pong")
	})
	...
	t.Run("pingReceivedPongReceived", func(t *testing.T) {
		...
		assert.Equal(t, false, pingReceived1 && pingReceived2, "only one side receives the ping")
		assert.Equal(t, false, pongReceived1 && pongReceived2, "only one side receives the pong")
		assert.Equal(t, true, (pingReceived1 && pongReceived2) || (pingReceived2 && pongReceived1), "ping and pong received")
	})
	...
	t.Run("pingReceivedPongNotReceived", func(t *testing.T) {
		...
		assert.Equal(t, false, pingReceived1 && pingReceived2, "only one side receives the ping")
		assert.Equal(t, true, (pingReceived1 && !pongReceived2) || (pingReceived2 && !pongReceived1), "ping received and pong not received")
	})
	...
	t.Run("netConn", func(t *testing.T) {
		...
		assert.Equal(t, n1.RemoteAddr(), n1.LocalAddr(), "remote addr")
		assert.Equal(t, "pipe", n1.RemoteAddr().String(), "remote addr string")
		assert.Equal(t, "pipe", n1.RemoteAddr().Network(), "remote addr network")
		...
		_, err = n1.Read(nil)
		assert.Equal(t, err, io.EOF, "read error")
		...
		assert.Equal(t, []byte("hello"), b, "read msg")
	})
	...
	t.Run("netConn/BadMsg", func(t *testing.T) {
		...
		_, err := io.ReadAll(n1)
		assert.ErrorContains(t, err, `unexpected frame type read (expected MessageBinary): MessageText`)
	...
	t.Run("netConn/readLimit", func(t *testing.T) {
		...
		assert.Equal(t, s, string(b), "read msg")
	})
	...
	t.Run("wsjson", func(t *testing.T) {
		...
		werr := xsync.Go(func() error {
			return wsjson.WriteJSON(tt.ctx, c1, exp)
		})

		var act any
		err := wsjson.ReadJSON(tt.ctx, c1, &act)
		assert.NoError(t, err)
		assert.Equal(t, exp, act, "read msg")
	...
	t.Run("CloseNow", func(t *testing.T) {
		...
		err1 = c1.CloseNow()
		err2 = c2.CloseNow()
		assert.ErrorIs(t, err1, websocket.ErrClosed)
		assert.ErrorIs(t, err2, websocket.ErrClosed)
	})
	...
	t.Run("MidReadClose", func(t *testing.T) {
		...
		err := wsjson.WriteJSON(tt.ctx, c1, "four")
		assert.NoError(t, err)
	...
	t.Run("ReadLimitExceededReturnsErrMessageTooBig", func(t *testing.T) {
		...
		assert.ErrorIs(t, err, websocket.ErrMessageTooBig)
		assert.ErrorContains(t, err, "read limited at 1025 bytes")
		...
		b.Run(bc.name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				...
				if websocket.MessageText != typ {
					assert.Equal(b, websocket.MessageText, typ, "data type")
				}
				...
				if err != io.EOF {
					assert.Equal(b, io.EOF, err, "read err")
				}
				if n2 != 0 {
					assert.Equal(b, 0, n2, "n2")
				}

				if !bytes.Equal(msg, readBuf) {
					assert.Equal(b, msg, readBuf, "msg")
				}
...
func assertEcho(tb testing.TB, ctx context.Context, c *websocket.Conn) {
	...
	werr := xsync.Go(func() error {
		return wsjson.WriteJSON(ctx, c, exp)
	})

	var act any
	c.SetReadLimit(1 << 30)
	err := wsjson.ReadJSON(ctx, c, &act)
	assert.NoError(tb, err)
	assert.Equal(tb, exp, act, "read msg")
...
func TestConcurrentClosePing(t *testing.T) {
	...
	for range 64 {
		func() {
			...
			assert.NoError(t, c1.Close(websocket.StatusNormalClosure, ""))
			<-errc
		}()
	}
}
