...
//   $ bundle -o=h2_bundle.go -prefix=http2 -tags=!nethttpomithttp2 github.com/malivvan/http/http2
...
// http2Priority is a PRIORITY frame to be sent on connection establishment.
type http2Priority struct {
	// StreamID is the stream that this frame applies to.
	StreamID uint32
	// PriorityParam is the priority of the stream.
...
func (s *http2Server) ServeConn(c net.Conn, opts *http2ServeConnOpts) {
	...
	sc.clientFingerprint = &HTTP2Fingerprint{}
	sc.fingerprint = &Fingerprint{HTTP2: sc.clientFingerprint}
	if tconn, ok := tlsConnOf(c); ok {
		sc.fingerprint.TLS = FingerprintFromTLSConn(tconn)
	}
...
type http2serverConn struct {
	...
	// clientFingerprint captures the HTTP/2 connection behavior of the
	// client for fingerprinting purposes. It is written by the serve loop
	// (see serveG) and deep-copied onto each Request.Fingerprint.
	clientFingerprint *HTTP2Fingerprint
	...
	fingerprint      *Fingerprint                // connection fingerprint (TLS + HTTP2), shared by all handlers
...
func (sc *http2serverConn) processWindowUpdate(f *http2WindowUpdateFrame) error {
	...
	if f.StreamID == 0 {
		...
		// fingerprinting. Bounded by MaxFingerprintFrames.
		if sc.clientFingerprint != nil && len(sc.clientFingerprint.WindowUpdates) < MaxFingerprintFrames {
...
func (sc *http2serverConn) processSettings(f *http2SettingsFrame) error {
	...
	// distinguishing client characteristic. Bounded by
	// MaxFingerprintSettings to limit memory usage.
	if sc.clientFingerprint != nil {
		f.ForeachSetting(func(s http2Setting) error {
			if len(sc.clientFingerprint.Settings) >= MaxFingerprintSettings {
				return nil
			}
			sc.clientFingerprint.Settings = append(sc.clientFingerprint.Settings, Setting{ID: SettingID(s.ID), Val: s.Val})
			return nil
		})
	}
...
func (sc *http2serverConn) processPriority(f *http2PriorityFrame) error {
	...
	// Record the PRIORITY frame for fingerprinting. Bounded by
	// MaxFingerprintFrames.
	if sc.clientFingerprint != nil && len(sc.clientFingerprint.Priorities) < MaxFingerprintFrames {
		sc.clientFingerprint.Priorities = append(sc.clientFingerprint.Priorities, Priority{
			StreamID: f.StreamID,
...
type http2Transport struct {
	...
	// Priorities, if non-nil, are the PRIORITY frames to send immediately
	// after the initial SETTINGS frame when a new connection is created.
	// Browsers send a fixed set of PRIORITY frames at connection start,
	// which is part of their HTTP/2 fingerprint.
	Priorities []http2Priority

	// HeaderPriority, if non-nil, is the stream priority to attach to the
	// HEADERS frame of every request. Browsers attach a fixed priority to
	// their request HEADERS frames, which is part of their HTTP/2
	// fingerprint.
	HeaderPriority *http2PriorityParam

	// ConnectionFlow is the connection-level flow control window to
	// advertise with the initial WINDOW_UPDATE frame. Zero means the
	// default (1<<30). Browsers advertise a fixed connection window
	// (e.g. 15663105 for Chrome), which is part of their HTTP/2
	// fingerprint.
...
func http2configureTransports(t1 *Transport) (*http2Transport, error) {
	...
	// Propagate the browser-impersonation HTTP/2 settings of the
	// http.Transport to the bundled http2 transport.
	if t1.HTTP2Settings != nil {
		for _, s := range t1.HTTP2Settings {
			t2.Settings = append(t2.Settings, http2Setting{ID: http2SettingID(s.ID), Val: s.Val})
		}
	}
	t2.HeaderTableSize = t1.HTTP2HeaderTableSize
	t2.InitialWindowSize = t1.HTTP2InitialWindowSize
	if t1.HTTP2MaxHeaderListSize != 0 && t2.MaxHeaderListSize == 0 {
		t2.MaxHeaderListSize = t1.HTTP2MaxHeaderListSize
	}
	for _, p := range t1.HTTP2Priorities {
		t2.Priorities = append(t2.Priorities, http2Priority{
			StreamID: p.StreamID,
			PriorityParam: http2PriorityParam{
	...
	if p := t1.HTTP2HeaderPriority; p != nil {
		t2.HeaderPriority = &http2PriorityParam{
			StreamDep: p.StreamDep,
			Exclusive: p.Exclusive,
			Weight:    p.Weight,
		}
	}
	t2.ConnectionFlow = t1.HTTP2ConnectionFlow
...
func (t *http2Transport) newClientConn(c net.Conn, addr string, singleUse bool) (*http2ClientConn, error) {
	...
	if t.Settings == nil {
		// Default behavior: send SETTINGS_ENABLE_PUSH based on the
		// PushHandler, like net/http. When custom settings are provided,
		// the caller controls the exact SETTINGS frame (browsers send a
		// fixed frame); nothing is injected.
	...
	setMaxHeader := false
	sawHeaderTableSize, sawInitialWindowSize := false, false
	if t.Settings != nil {
	...
	} else if !sawHeaderTableSize {
		initialSettings = append(initialSettings, http2Setting{ID: http2SettingHeaderTableSize, Val: http2initialHeaderTableSize})
	}
	if t.Settings == nil {
		// With default settings, advertise the (default or configured)
		// SETTINGS_MAX_HEADER_LIST_SIZE. With custom settings the caller
		// controls the exact frame.
	...
	connFlow := int32(http2transportDefaultConnFlow)
	if t.ConnectionFlow != 0 {
		connFlow = int32(t.ConnectionFlow)
	}
	cc.fr.WriteWindowUpdate(0, uint32(connFlow))
	cc.inflow.add(connFlow + http2initialWindowSize)
	cc.bw.Flush()
	if cc.werr != nil {
		cc.Close()
	...
	for len(hdrs) > 0 && cc.werr == nil {
		...
		if first {
			...
			if cc.t.HeaderPriority != nil {
				p.Priority = *cc.t.HeaderPriority
			}
			cc.fr.WriteHeaders(p)
			first = false
		} else {
